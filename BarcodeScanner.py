...
def scan_barcode(frame,log_results=False,log_file="scanned_barcodes.txt"):
    ...
    for barcode in barcodes:
        ...
        cv2.polylines(frame,[points],True, color=(0,255,0),thickness=3)
        rect=barcode.rect
...
def save_to_file(data,barcode_type,file_path):
    with open(file_path,'a') as f:
        ...
        f.write("{},{},{}\n".format(timestamp,barcode_type,data))
...
def start_scanner():
    ...
    while True:
        ...
        cv2.imshow(" Barcodes Scanner",scanned_frame)
...
def select_file():
    ...
    if file_path:
        ...
        cv2.imshow(" Barcodes Scanner",scanned_image)
        if scanned_data:
            save_option=messagebox.askyesno("saved results","do you want to save results?")
            if save_option:
...
def save_results_to_file(scanned_data,source_file):
    ...
    messagebox.showinfo("Saved Results","saved results to {}".format(log_file))

def view_log_file():
    log_file="scanned_results_{}.txt"
    if not  os.path.exists(log_file):
        messagebox.showwarning("No Logs","Log file not found.")
        return
    ...
    log_text=tk.Text(log_window,wrap='word',width=20,height=60)
    log_text.insert('1.0',''.join(logs))
    log_text.config(state='disabled')
    log_text.pack(padx=10,pady=10)

app=tk.Tk()
app.title("Barcodes Scanner")
...
footer_label = tk.Label(app, text="Press 'q' to exit Scanner", fg="blue")
